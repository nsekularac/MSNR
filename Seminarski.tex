...
box
\end{verbatim} operatora koji uzima ime i funkciju koja generiše kolo kao ulaz. Ovako ''spakovane'' funkcije mogu biti i ugnežđene (eng.~\emph{nested}).
...
U Quipperu, isti opis kvantnog kola se može koristiti za konstrukciju i manipulaciju u memoriji ili za njegovo izvršenje. Na primer, funkcija \begin{verbatim} run_generic \end{verbatim} simulira kolo dok funkcija \begin{verbatim} print_generic \end{verbatim} štampa kolo u nekom od izlaznih formata.

\section{Zaključak}

 U ovom kratkom radu pokusali smo da sto jednostavnije i sažetije predstavimo sam koncept kvantnog funkcionalnog programiranja i neke jezike koji se time bave, a zbog obima rada, nismo mogli da se pozabavimo još nekim kvantnim funkcionalnim jezicima kao što su Quantum lambda calculi i QML. 
Za buduće revizije ostaje realizacija nekih primera upotrebe ovih jezika kako bi demonstrirali njihovu upotrebljivost.
...
\end{document}
>>>>>>> Update quippera, sitne prepravke
